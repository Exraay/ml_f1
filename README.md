...
If you are reproducing results from a **CSV-only** dataset (e.g., committee review):
1. Place the CSV at `data/processed/laps_base.csv`.
2. Ensure `data/processed/laps_base.metadata.json` exists (created by `--build-data`).
3. Set `DATA_FORMAT = "csv"` in `notebooks/_common.py` before running notebooks.
...
## Key plots (GitHub preview)

These figures are generated by `notebooks/06_model_comparison_full.ipynb`.

**Overall performance (MAE vs RMSE)**
![Model comparison MAE vs RMSE](reports/notebooks/06_model_comparison_full/model_comparison_mae_rmse.png)

**Cumulative error distribution (2025 test)**
![Cumulative error distribution](reports/notebooks/06_model_comparison_full/cumulative_error_distribution.png)

**Accuracy within thresholds**
![Accuracy thresholds](reports/notebooks/06_model_comparison_full/accuracy_thresholds.png)

**MAE by round (2025, all models)**
![MAE by round 2025](reports/notebooks/06_model_comparison_full/mae_by_round_2025.png)

**XGBoost feature importance (top 20)**
![XGBoost feature importance](reports/notebooks/06_model_comparison_full/feature_importance_xgboost.png)
